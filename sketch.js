...
//
// This file is the one copy of the sketch. It hands p5 an instance-mode factory
// on window.albersSketch rather than starting itself, so a page decides when to
// run it and keeps the handle to stop it: index.html next door starts one for
// the page here, and the Albers post on bobbymeyer.com loads this same file
// from the deploy and starts its own, which it tears down on navigation.
...
(function () {
  ...
  // The palette is built in OKLab, where a step in lightness is roughly a step
  // in what the eye sees, so we can promise every touching pair of squares a
  // visible edge. MIN_STEP is that promise: the smallest OKLab lightness gap
  // allowed between neighbours. The floor and ceiling keep the ramp off the two
  // ends where a screen crushes everything into black or white.
  const MIN_STEP = 0.11;
  const L_FLOOR = 0.32;
  const L_CEIL = 0.94;
  ...
  // into something p5 can fill with. Plain maths, so it sits outside the sketch.
  ...
  function oklchToRgb(l, c, h) {
    return oklchToLinearRgb(l, c, h).map((v) => clamp(toSrgb(clamp(v, 0, 1)) * 255, 0, 255));
  }

  // --- The sketch -----------------------------------------------------------

  function albersSketch(p) {
    let rotation = 0;
  ...
  window.albersSketch = albersSketch;
})();
